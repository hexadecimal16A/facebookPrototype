	...
	$(document).ready(function() {
		...
		modalform2.submit(function() {
			...
			$.ajax({
				...
				url : modalform2.attr('action') + "/" + sessionStorage.event_id+"_"+sessionStorage.id,
				data : fd,
		...
		$.ajax({
			type : "get",
			url : "webapi/eventfeed/" + sessionStorage.event_id,
			success : function(data) {
				if (data) {
					//console.log(data);
					for(i=0;i<data.length;++i){
						...
						$.ajax({
							type:'get',
							url:'webapi/elcm/likes?post_id='+post_id+'&user_id='+sessionStorage.id,
							async:false,
							success:function(data){
							...
							complete:function(){
								$.ajax({
									type:'get',
									url:'webapi/elcm/comments?post_id='+post_id,
									async:false,
									success:function(data){
			...
			complete: function(){
				...
				$('a[id^=like_]').click(function(e){
					...
					$.ajax({
						type:'put',
						url:'webapi/elcm/likes?post_id='+($(this).attr('id')).split("_")[1]+'&user_id='+sessionStorage.id,
						async:false,
						success:function(data){
							if(data){
				...
				$('a[id^=comment_]').click(function(e){
					...
					commentPost.submit(function() {
						...
						$.ajax({
							type: 'put',
							url: 'webapi/elcm/comments?post_id='+id+'&user_id='+sessionStorage.id,
