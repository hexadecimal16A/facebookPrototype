...
public class SqlGroupLikescommentshare {
	...
	public boolean putLike(Long post_id,Long user_id) throws SQLException{
		try{
		conn=makeSQLConn();
		sqlQuery="insert into "+groupLikeTable+" (post_id,user_id) values (?,?)";
		...
		return true;
			...
			sqlQuery="delete from "+groupLikeTable+" where post_id=? and user_id=?";
			preparedStatement=conn.prepareStatement(sqlQuery);
			preparedStatement.setLong(1,post_id);
			preparedStatement.setLong(2,user_id);
			preparedStatement.executeUpdate();
	...
	public String getLike(Long post_id,Long user_id){
		...
		try{
			conn=makeSQLConn();
			sqlQuery="select * from "+groupLikeTable+" where post_id=? and user_id=?";
			preparedStatement=conn.prepareStatement(sqlQuery);
			preparedStatement.setLong(1,post_id);
			preparedStatement.setLong(2,user_id);
			...
			if(!rs.isBeforeFirst())
				...
				outString="true";
			sqlQuery="select count(*) from "+groupLikeTable+" where post_id=?";
			preparedStatement=conn.prepareStatement(sqlQuery);
			preparedStatement.setLong(1,post_id);
	...
	public ArrayList<SqlUserProfileData> getLikeInfo(Long post_id) throws SQLException{
		...
		sqlQuery="select fname,lname,id,picturepath from "+userTable+" inner join  "
				+groupLikeTable+" on "+groupLikeTable+".user_id="+userTable+".id where post_id=?";
		preparedStatement=conn.prepareStatement(sqlQuery);
	...
	public String getComments(Long post_id) throws SQLException{
		...
		try{
			...
			sqlQuery="select count(*) from "+groupCommentTable+" where post_id=?";
			preparedStatement=conn.prepareStatement(sqlQuery);
			preparedStatement.setLong(1,post_id);
			ResultSet rs=preparedStatement.executeQuery();
			if(!rs.isBeforeFirst()){
	...
	public ArrayList<CommentsData> getCommentsInfo(Long post_id) throws SQLException{
		...
		sqlQuery="select fname,lname,id,picturepath,commenttext,comment_id from "
				+userTable+","+groupCommentTable+" where "+groupCommentTable
	...
	public boolean putComment(Long post_id,Long user_id,String commenttext) throws SQLException{
		try{
		conn=makeSQLConn();
		sqlQuery="insert into "+groupCommentTable+" (post_id,user_id,commenttext,creation_time) values (?,?,?,?)";
		preparedStatement=conn.prepareStatement(sqlQuery);
		preparedStatement.setLong(1,post_id);
		preparedStatement.setLong(2,user_id);
